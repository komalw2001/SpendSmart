...
public class AddExpenseOrIncome extends AppCompatActivity {
    ...
    private void showConfirmationDialog() {
        AlertDialog.Builder builder = new AlertDialog.Builder(AddExpenseOrIncome.this);
                                                ...
                                                dbRef.child("Users")
                                                        ...
                                                        .addOnSuccessListener(new OnSuccessListener<Void>() {
                                                            ...
                                                            public void onSuccess(Void unused) {
                                                                ...
                                                                updateBudgets(user, Double.parseDouble(amt), selectedCategoryIndex);
                                                            }
                                                        }).addOnFailureListener(new OnFailureListener() {
                                                            @Override
                                                            public void onFailure(@NonNull Exception e) {
                                                                Log.d("Updating Balance",e.getMessage());
    ...
    private void updateBudgets(String user, double expenseAmount, int expenseCategory) {
        Query budgetQuery = dbRef.child("Budget").orderByChild("user").equalTo(user);
        budgetQuery.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot budgetSnapshot : snapshot.getChildren()) {
                    Budget budget = budgetSnapshot.getValue(Budget.class);
                    if (budget != null) {
                        if (budget.getCategoryIndex() == expenseCategory) {

                            double newTotalSpent = budget.getTotalSpent() + expenseAmount;
                            budgetSnapshot.getRef().child("totalSpent").setValue(newTotalSpent);

                            //  totalSpent exceeds totalBudget
                            if (newTotalSpent > budget.getTotalBudget()) {
                                Calendar calendar = Calendar.getInstance();
                                Date currentDate = calendar.getTime();
                                SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
                                String formattedDate = dateFormat.format(currentDate);

                                HashMap<Object, Object> reminderData = new HashMap<>();
                                reminderData.put("type", 1);
                                reminderData.put("title", "Budget Exceeded!");
                                reminderData.put("text", "Budget exceeded: " + budget.getBudgetName());
                                reminderData.put("date", formattedDate);
                                ...
                                dbRef.child("Reminders")
                                        ...
                                        .addOnSuccessListener(new OnSuccessListener<Void>() {
                                            @Override
                                            public void onSuccess(Void unused) {
                                                Log.d("Adding Reminder", "Successfully added budget exceeded reminder");
                                                Toast.makeText(getBaseContext(),"Budget exceeded!",Toast.LENGTH_SHORT).show();
                                            }
                                        })
                                        .addOnFailureListener(new OnFailureListener() {
                                            @Override
                                            public void onFailure(@NonNull Exception e) {
                                                Log.d("Adding Reminder", e.getMessage());
                                            }
                                        });
                            }
                        }
            ...
            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Log.d("Budgets Listener", "Error getting budgets");
            }
        });
    }
